...
// RECOMPHAMR_NO_UPDATE_CHECK=="1" even when a user already exported a different
...
func TestReexecGuardOverridesPreexistingValue(t *testing.T) {
	t.Setenv("RECOMPHAMR_NO_UPDATE_CHECK", "0") // user set it wrong; restored after test
	os.Setenv("RECOMPHAMR_NO_UPDATE_CHECK", "1")
	if got := os.Getenv("RECOMPHAMR_NO_UPDATE_CHECK"); got != "1" {
