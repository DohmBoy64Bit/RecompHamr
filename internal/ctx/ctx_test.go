...
func TestMessageTokensCountsToolCallArguments(t *testing.T) {
	base := Message{Role: RoleAssistant, ToolCalls: []ToolCall{{Name: "powershell"}}}.Tokens()
	withArgs := Message{Role: RoleAssistant, ToolCalls: []ToolCall{
		{Name: "powershell", Arguments: map[string]any{"script": "echo hello world"}},
	}}.Tokens()
	// args add Tokens("script")=2 + Tokens(fmt.Sprint("echo hello world"))=4 = 6.
	if got := withArgs - base; got != 6 {
		t.Fatalf("argument cost = %d, want 6 (Message.Tokens must account for ToolCall.Arguments)", got)
	}
}
...
func TestPackDropsOrphanToolMessage(t *testing.T) {
	...
	history := []Message{
		{Role: RoleAssistant, ToolCalls: []ToolCall{{ID: "c1", Name: "powershell"}}},
...
func TestPackDropsEmptyIDToolMessages(t *testing.T) {
	history := []Message{
		...
		{Role: RoleAssistant, ToolCalls: []ToolCall{{ID: "", Name: "powershell"}}},
...
func TestPackKeepsNewestParallelToolGroupOverBudget(t *testing.T) {
	...
	history := []Message{
		...
		{Role: RoleAssistant, ToolCalls: []ToolCall{
			{ID: "c1", Name: "powershell", Arguments: map[string]any{"script": bigArgs}},
			{ID: "c2", Name: "powershell"},
		}},
...
func TestPackDropsDanglingPartialParallelGroup(t *testing.T) {
	history := []Message{
		{Role: RoleUser, Content: "do two things"},
		{Role: RoleAssistant, ToolCalls: []ToolCall{
			{ID: "c1", Name: "powershell"}, {ID: "c2", Name: "powershell"},
		}},
...
func TestPackDropsDanglingAssistantDespiteIDReuse(t *testing.T) {
	history := []Message{
		...
		{Role: RoleAssistant, ToolCalls: []ToolCall{{ID: "call_0", Name: "powershell"}}},
...
func TestPackDropsStrayToolResultDespiteIDReuse(t *testing.T) {
	history := []Message{
		...
		{Role: RoleAssistant, ToolCalls: []ToolCall{
			{ID: "call_0", Name: "powershell"}, {ID: "call_1", Name: "powershell"},
		}},
