...
const (
	ToolOutputCap = 6000
	ToolHeadTail  = 2000
	// FixedSystem is intentionally preserved from the pinned CodeHamr baseline
	// even though this stripped template currently embeds a smaller system prompt.
	// Keeping the reservation unchanged avoids silently changing context-packing
	// behavior before the baseline gate closes. A test still verifies that the
	// live prompt plus working-directory anchor fits inside the reservation.
	FixedSystem = 4200
	FixedTools  = 1500
)
