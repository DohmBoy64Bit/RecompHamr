...
**Browser / canvas / WebGL / GUI:** running a web/UI/canvas artifact headless *is* the verification, not optional polish — reading the source won't catch an undefined variable or a shader that won't compile, and a static check is **not** proof it renders. Two rungs, in order:

1. **Syntax-check the inline script as a module** — one shot: `sed -n '/<script type="module">/,/<\/script>/p' page.html | sed '1d;$d' | node --check --input-type=module` (or save the script `.mjs` and `node --check that.mjs`). Never trust bare `node --check file.js`: after a top-level `import` it falls back to CommonJS and **exits 0 on the very `Unexpected token` it should catch** — the false green that ships broken pages. `--input-type=module` needs stdin (the `<`/pipe), never a filename.
2. **Then actually run it.** Find a browser even off `PATH`: `command -v chromium chromium-browser google-chrome 2>/dev/null; ls ~/.cache/ms-playwright/*/chrome-linux/{chrome,headless_shell} 2>/dev/null` (playwright caches a usable binary there). Serve the file, load it, read the console: `<chrome> --headless --no-sandbox --dump-dom --virtual-time-budget=8000 --enable-logging=stderr --v=1 URL 2>err.txt; grep CONSOLE err.txt` — an `Uncaught`/error among those lines is the bug. For WebGL on a GPU-less box add `--use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader` (recent Chrome dropped the silent SwiftShader fallback — without them the canvas is blank and you'd wrongly call it broken). This loop is also how you polish past "renders" to "looks right".

Asserting a string is present, counting braces (shader/template-literal braces unbalance legally — you'll chase a phantom), re-simulating the JS in another language, or `curl`-ing for a 200 prove the file *exists*, not that it *runs* — none is verification. If there's no browser anywhere, your summary must say so in one plain line ("I couldn't run it headless here — please open it") and never let a passing syntax check or a 200 stand in for "it renders".
