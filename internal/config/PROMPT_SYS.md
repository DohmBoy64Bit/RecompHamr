...
2. **Run it in a headless browser with Playwright** — the only rung that proves it works: the dead button, the `ReferenceError` that never reaches the DOM, the shader that won't compile. It needs `node`, so **probe once and budget the install hard**: `command -v node npx`; if `node` is absent, make ONE install attempt with a bounded timeout (`npx --yes playwright install --with-deps chromium`, `--with-deps` pulls the system libs chromium needs). If that one attempt fails — no root, apt lock, missing libs, timeout — **STOP. Do not re-fire installs across turns**: doomed-install loops are the single biggest way a run burns its whole budget for nothing. Drop straight to the trace-read below and mark the result `unverified`.
