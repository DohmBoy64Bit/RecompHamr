...
#### Checkpoint 4D — agent lifecycle migration

- Changed: routed goal submission, model-round reads, stream reduction, retry/content/flush presentation, sequential tool execution, close decisions, cancellation, stale model draining, stale tool rejection, and final accounting through the app controller. Deleted TUI agent/runtime fields, backend-shaped messages, lifecycle helpers, tool execution commands, and production `internal/agent` imports.
- Documented: frontend event contracts, controller ownership, current architecture, and this checkpoint agree with the implementation. No visible command, layout, persistence, provider, tool, or agent-policy contract changed.
- Verified: controller contract tests cover clean/retry/error streams, duplicate submit, ordered multiple tools, stale tool completion, empty-reply retry/stall, tool-call leakage, cancellation, stale non-closed stream draining, exactly-once completion, and final accounting. Fake-controller TUI tests cover startup/history, every presentation event, opaque completion return, queue restore/auto-fire, phase/status/footer/transcript changes, and input intent emission. The canonical gate passes at exactly 100.0% statements.
- Coverage: production TUI imports only `internal/frontend` among project runtime packages; source inspection finds no agent/session/config/context/LLM/provider/tool/logging import, backend delivery, mutable runtime alias, or lifecycle call. Controller and TUI focused packages each pass at 100.0% statements.
- Security: frontend events expose bounded display text and aggregate timing/token/profile facts only. Model history, reasoning, raw transport, tool arguments/results, process handles, contexts/cancel functions, credentials, observers, and log contents remain private to application/backend owners.
- Evidence: Windows Terminal scenarios `smoke`, `model-stream-cancel`, and `agent-tool-loop-cancel` passed against the verified executable with LM Studio `google/gemma-4-12b-qat`. Sanitized reports and reviewed screenshots are retained under `E:\ReProject\StageA-Acceptance\StageC-Slice4`; the tool scenario proves exactly four ordered accepted tool results, cancelled PowerShell cleanup, absent side effect, no stale result/reissue, recovery, and restored shell.
- Known limits: core/terminal composition isolation, final architecture enforcement and positive deletion build, exact implementation-commit rerun, representative 120×36/80×24/50×16 closure images, CI, and final Stage C documentation/status remain open.

#### Checkpoint 4E — composition and deletion boundary

- Changed: core `internal/app` now composes backend services and returns only a neutral controller lifetime with idempotent cleanup. Added `internal/app/terminal` as the sole owner of `tui.New`, Bubble Tea creation/execution, focus reporting, inline clearing, and terminal help formatting. The process entrypoint delegates only to this terminal adapter.
- Documented: current and target architecture now distinguish core application behavior from the literal runnable terminal target; the deletion proof applies to core/backend packages while the terminal adapter remains the intentional presentation edge.
- Verified: strengthened architecture enforcement rejects backend-to-TUI imports, core-app-to-TUI/Bubble-Tea imports, production TUI backend imports/lifecycle symbols, frontend backend/Bubble-Tea imports, and entrypoint bypass. Its positive `go list -deps` proof confirms core app/controller/frontend/agent/session/config/context/LLM/provider/tools/logging resolve without `internal/tui` or Bubble Tea. Focused app/terminal tests and the canonical gate pass at exactly 100.0% statements.
- Coverage: core bootstrap success/failure, environment override, logging/no-logging cleanup, absolute-path fallback, controller-only exposure, repeated close, terminal bootstrap/write/program failures, help writer failures, default factories, and concrete Tea construction are covered.
