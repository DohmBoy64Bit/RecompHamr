#!/usr/bin/env bash
# RecompHAMR installer: fetch the latest release binary and install it into a
# user-writable prefix so sudo is never needed for the default flow.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/DohmBoy64Bit/RecompHamr/main/install.sh | bash
#   curl -fsSL https://raw.githubusercontent.com/DohmBoy64Bit/RecompHamr/main/install.sh | PREFIX=/usr/local bash

set -euo pipefail

clear

REPO="DohmBoy64Bit/RecompHamr"

os="$(uname -s | tr '[:upper:]' '[:lower:]')"
case "$os" in
  linux)  ;;
  darwin) os=macos ;;
  *) echo "recomphamr: unsupported OS: $os (need linux or darwin)" >&2; exit 1 ;;
esac
...
case "$arch" in
  ...
  *) echo "recomphamr: unsupported arch: $arch (need amd64 or arm64)" >&2; exit 1 ;;
esac
...
binary="recomphamr-${os}-${arch}"
url="https://github.com/${REPO}/releases/latest/download/${binary}"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT

echo "▶ recomphamr · ${os}/${arch}"
curl -fsSL "$url" -o "${tmp}/recomphamr"

install -d "${bindir}" 2>/dev/null \
  || { echo "recomphamr: cannot create ${bindir} · pick a writable PREFIX, e.g. PREFIX=\$HOME/.local bash" >&2; exit 1; }
install -m 0755 "${tmp}/recomphamr" "${bindir}/recomphamr" 2>/dev/null \
  || { echo "recomphamr: cannot write ${bindir}/recomphamr · pick a writable PREFIX, e.g. PREFIX=\$HOME/.local bash" >&2; exit 1; }

echo "✓ installed → ${bindir}/recomphamr"
...
if ! on_path "$bindir"; then
  ...
  echo "    ${line}"
  echo ""
  echo "  then type 'recomphamr' to start hammering"
else
  echo ""
